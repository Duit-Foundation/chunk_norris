...
void main() {
  group('ChunkField', () {
    ...
    group('value access', () {
      ...
      test('should throw StateError when accessing value after error',
          () async {
        final field = ChunkField<int>('test-id');
    ...
    group('deserialization', () {
      ...
      test('should throw ArgumentError when no deserializer and types mismatch',
          () async {
        final field = ChunkField<int>('test-id');
    ...
    group('toString', () {
      test('should show pending state', () {
        ...
        expect(
            field.toString(), equals('ChunkField<String>(pending: \$test-id)'));
      });
      ...
      test('should show loaded state with value', () {
        ...
        expect(
            field.toString(), equals('ChunkField<String>(loaded: test value)'));
      });
    ...
    group('edge cases', () {
      test('should handle null data with deserializer', () {
        final field =
            ChunkField<String>('test-id', (data) => data?.toString() ?? 'null');
      ...
      test('should handle deserializer that returns null', () {
        ...
        field.resolve(
            42); // Используем не-String тип, чтобы активировать десериализатор
